...
void MainWindow::loadFile(const QString& path) {
    ...
    else { const QByteArray encoded = absolute.toUtf8(); const char* args[] = {"loadfile", encoded.constData(), "replace", nullptr}; mpv_command_async(m_mpv, 0, args); }
}
...
void MainWindow::addToPlaylist(const QString& path) {
    ...
    for (int i = 0; i < m_playlist->count(); ++i) {
        if (m_playlist->item(i)->data(Qt::UserRole).toString() == absolute) { m_playlist->setCurrentRow(i); return; }
    }
...
void MainWindow::syncPlaylistSelection() { if (m_playlist && m_currentPlaylistIndex >= 0 && m_currentPlaylistIndex < m_playlist->count()) m_playlist->setCurrentRow(m_currentPlaylistIndex); }
...
void MainWindow::showTracksMenu() {
    ...
    auto* audioMenu = menu->addMenu(QStringLiteral("Audio")); auto* subtitleMenu = menu->addMenu(QStringLiteral("Subtitles"));
    const mpv_node_list* list = tracks.u.list; bool hasAudio = false; bool hasSubtitles = false;
    auto addTrack = [this](QMenu* target, const QString& label, int id, bool selected, const char* property) {
        auto* action = target->addAction(label); action->setCheckable(true); action->setChecked(selected);
        connect(action, &QAction::triggered, this, [this, id, property] { if (id < 0) { static char noTrack[] = "no"; char* value = noTrack; mpv_set_property_async(m_mpv, 0, property, MPV_FORMAT_STRING, &value); } else { int64_t value = id; mpv_set_property_async(m_mpv, 0, property, MPV_FORMAT_INT64, &value); } });
    };
    addTrack(audioMenu, QStringLiteral("Auto"), -1, false, "aid");
    mpv_node aid{}; if (mpv_get_property(m_mpv, "aid", MPV_FORMAT_NODE, &aid) >= 0 && aid.format == MPV_FORMAT_INT64) audioMenu->actions().first()->setChecked(aid.u.int64 < 0); mpv_free_node_contents(&aid);
    auto* autoSubtitle = subtitleMenu->addAction(QStringLiteral("Off")); autoSubtitle->setCheckable(true);
    mpv_node sid{}; if (mpv_get_property(m_mpv, "sid", MPV_FORMAT_NODE, &sid) >= 0 && sid.format == MPV_FORMAT_INT64) autoSubtitle->setChecked(sid.u.int64 < 0); mpv_free_node_contents(&sid);
    connect(autoSubtitle, &QAction::triggered, this, [this] { static char noSubtitle[] = "no"; char* value = noSubtitle; mpv_set_property_async(m_mpv, 0, "sid", MPV_FORMAT_STRING, &value); });
    for (int i = 0; i < list->num; ++i) {
        const mpv_node& track = list->values[i]; if (track.format != MPV_FORMAT_NODE_MAP || !track.u.list) continue;
        const QString type = nodeString(mapValue(track.u.list, "type")); const int id = nodeInt(mapValue(track.u.list, "id")); if (id < 0) continue;
        const QString lang = nodeString(mapValue(track.u.list, "lang")); const QString title = nodeString(mapValue(track.u.list, "title")); const QString external = nodeString(mapValue(track.u.list, "external-filename")); const bool selected = nodeFlag(mapValue(track.u.list, "selected"));
        QString label = title; if (label.isEmpty()) label = lang; if (label.isEmpty() && !external.isEmpty()) label = QFileInfo(external).fileName(); if (label.isEmpty()) label = QStringLiteral("Track %1").arg(id); if (!lang.isEmpty() && title != lang) label += QStringLiteral(" (%1)").arg(lang);
        if (type == QStringLiteral("audio")) { addTrack(audioMenu, label, id, selected, "aid"); hasAudio = true; }
        else if (type == QStringLiteral("sub")) { auto* action = subtitleMenu->addAction(label); action->setCheckable(true); action->setChecked(selected); connect(action, &QAction::triggered, this, [this, id] { int64_t value = id; mpv_set_property_async(m_mpv, 0, "sid", MPV_FORMAT_INT64, &value); }); hasSubtitles = true; }
    }
    audioMenu->setEnabled(hasAudio); subtitleMenu->setEnabled(hasSubtitles || subtitleMenu->actions().size() > 0);
    if (auto* button = qobject_cast<QPushButton*>(sender())) menu->popup(button->mapToGlobal(QPoint(0, button->height()))); else menu->popup(QCursor::pos());
...
void MainWindow::keyPressEvent(QKeyEvent* event) { switch (event->key()) { case Qt::Key_Space: togglePause(); break; case Qt::Key_Left: seekBackward(); break; case Qt::Key_Right: seekForward(); break; case Qt::Key_Up: playPrevious(); break; case Qt::Key_Down: playNext(); break; case Qt::Key_Plus: case Qt::Key_Equal: adjustVideoZoom(0.1); break; case Qt::Key_Minus: adjustVideoZoom(-0.1); break; case Qt::Key_Z: resetVideoTransform(); break; case Qt::Key_A: setAbLoopStart(); break; case Qt::Key_B: setAbLoopEnd(); break; case Qt::Key_L: clearAbLoop(); break; case Qt::Key_Period: stepFrame(true); break; case Qt::Key_Comma: stepFrame(false); break; case Qt::Key_F11: isFullScreen() ? showNormal() : showFullScreen(); break; case Qt::Key_Escape: if (isFullScreen()) showNormal(); break; default: QMainWindow::keyPressEvent(event); break; } }

bool MainWindow::eventFilter(QObject* watched, QEvent* event) {
    if (watched != m_videoWidget) return QMainWindow::eventFilter(watched, event);
    if (event->type() == QEvent::MouseButtonDblClick) { const auto* e = static_cast<QMouseEvent*>(event); if (e->button() != Qt::LeftButton) return false; const qreal x = e->position().x(); const qreal width = m_videoWidget->width(); if (x < width/3.0) seekBackward(); else if (x > width*2.0/3.0) seekForward(); else togglePause(); return true; }
    if (event->type() == QEvent::MouseButtonPress) { const auto* e = static_cast<QMouseEvent*>(event); if (e->button() == Qt::MiddleButton) { m_panningVideo = true; m_panStart = e->position(); m_videoPanX = getPropertyDouble("video-pan-x"); m_videoPanY = getPropertyDouble("video-pan-y"); return true; } }
    if (event->type() == QEvent::MouseMove && m_panningVideo) { const auto* e = static_cast<QMouseEvent*>(event); const QPointF delta = e->position() - m_panStart; const double width = std::max(1, m_videoWidget->width()); const double height = std::max(1, m_videoWidget->height()); m_videoPanX = std::clamp(m_videoPanX + delta.x()/width, -1.0, 1.0); m_videoPanY = std::clamp(m_videoPanY + delta.y()/height, -1.0, 1.0); setPropertyDouble("video-pan-x", m_videoPanX); setPropertyDouble("video-pan-y", m_videoPanY); m_panStart = e->position(); return true; }
    if (event->type() == QEvent::MouseButtonRelease) { const auto* e = static_cast<QMouseEvent*>(event); if (e->button() == Qt::MiddleButton && m_panningVideo) { m_panningVideo = false; return true; } }
    if (event->type() == QEvent::Wheel) { const auto* e = static_cast<QWheelEvent*>(event); const int delta = !e->angleDelta().isNull() ? e->angleDelta().y() : e->pixelDelta().y(); if (delta == 0) return false; if (e->modifiers().testFlag(Qt::AltModifier)) adjustVideoZoom(delta > 0 ? 0.1 : -0.1); else if (e->modifiers().testFlag(Qt::ControlModifier)) m_volumeSlider->setValue(std::clamp(m_volumeSlider->value() + (delta > 0 ? 5 : -5), 0, 100)); else { const char* args[] = {"seek", delta > 0 ? "5" : "-5", "relative", "exact", nullptr}; command(args); } return true; }
